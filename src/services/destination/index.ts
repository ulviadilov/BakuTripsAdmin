...
const create = async (destinationData: RequestDestination) => {
  try {
    ...
    if (
      destinationData.Translations &&
      Array.isArray(destinationData.Translations)
    ) {
...
const getAll = async (page: number, size: number) => {
  try {
    const response = await apiClient.get(
      `/Destinations/get-all-destinations?page=${page}&size=${size}`
    );
    return response;
  } catch (error) {
...
const getById = async (id: string) => {
  try {
    const response = await apiClient.get(
      `/Destinations/${id}?includeAllTranslations=true`
    );
    return response.data;
  } catch (error) {
