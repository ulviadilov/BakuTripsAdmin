...
const schema = yup.object({
    ...
    vrimagefile: yup
        .mixed<File | string>()
        .required("VR image is required")
        .nullable(),
    travelPackageImages: yup
        .array()
...
export default function TourOptionEdit() {
    ...
    // Fetch package options for select dropdown
    const { data, isPending } = useQuery({
        queryKey: [QUERY_KEYS.packageOption.select],
        queryFn: packageTourService.getPackageSelect,
    });

    // Fetch existing tour option data
    const {
        data: tourOptionData,
        isLoading: isFetchingTourOption,
        error: fetchError,
    } = useQuery({
    ...
    useEffect(() => {
        if (tourOptionData) {
            ...
            setValue(
                "optionname",
                tourOptionData.data?.packageOption.optionName || ""
            );
            setValue("mapurl", tourOptionData.data?.packageOption.mapUrl || "");
            setValue(
                "shortdescription",
                tourOptionData.data?.packageOption.shortDescription || ""
            );
            setValue(
                "apartmentinfo",
                tourOptionData.data?.packageOption.apartmentInfo || ""
            );
            setValue(
                "roominfo",
                tourOptionData.data?.packageOption.roomInfo || ""
            );
            setValue(
                "vehicleinfo",
                tourOptionData.data?.packageOption.vehicleInfo || ""
            );
            setValue(
                "fulldescription",
                tourOptionData.data?.packageOption.fullDescription || ""
            );
            setValue("price", tourOptionData.data?.packageOption.price || 0);
            setValue(
                "importantinfos",
                tourOptionData.data?.packageOption.importantInfos || []
            );
            setValue(
                "includes",
                tourOptionData.data?.packageOption.includes || []
            );
            setValue(
                "excludes",
                tourOptionData.data?.packageOption.excludes || []
            );
            setValue(
                "vrimagefile",
                tourOptionData.data?.packageOption.vrImagePath || null
            );
            setValue(
                "travelPackageImages",
    ...
    const mutation = useMutation({
        mutationFn: (data: PackageTourOptionGet) =>
            packageOptionService.update(id!, data, newImagesFiles, removeIds),
        onSuccess: () => {
            toast.success("Tour option updated successfully");
            navigate(paths.PACKAGE_TOUR_OPTION.LIST);
        },
        onError: (error) => {
            console.error(error);
            toast.error("Failed to update tour option");
        },
    });
    ...
    const handleDelete = (id: string) => {
        const isExist =
            tourOptionData?.data?.packageOption?.travelPackageImages.find(
                ...
                <div className="flex items-center justify-center min-h-[400px]">
                    <div className="text-center">
                        ...
                        <h3 className="text-lg font-medium text-gray-900 mb-1">
                            Failed to load tour option data
                        </h3>
                        <p className="text-gray-500 mb-4">
                            There was an error loading the tour option
                            information.
                        </p>
                        <button
                            onClick={handleBack}
    ...
    const option = tourOptionData?.data?.packageOption;
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-semibold text-gray-900">
                            Edit Tour Package Option
                        </h1>
                        <p className="text-gray-600 text-sm mt-1">
                            Update tour package option details
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="bg-white p-6 rounded-lg border border-gray-200">
                        ...
                        <div className="space-y-6">
                            ...
                            <FileUpload
                                ...
                                required={true}
                                initialUrls={option.vrImagePath}
                            />
                            ...
                            <FileUpload
                                ...
                                required={true}
                                initialUrls={option.travelPackageImages}
                            />
                        </div>
                    </div>
